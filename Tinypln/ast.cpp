...
namespace cen
{
    ...
    VariableDefinitionStatementAST::VariableDefinitionStatementAST(NameASTPtr lhs, ExprASTPtr rhs, TokenLocation location)
    ...
    {
		const std::string& varName = lhs_->getName();
        if (env->lookupLocation(varName))
        {
            errorInterp("变量 " + varName + " 定义重复:" + tokenLocation_.toString());
            return nullptr;
        }
        auto rval = rhs_->eval(env);
        env->putLocationValue(varName, rval);
